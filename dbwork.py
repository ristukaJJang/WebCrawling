...
def get_detail_info(no):
    ...
    try:
        ...
        soup = BeautifulSoup(html, 'lxml')
        filtered_datas = {}
        company_summery = {}  # 각 기업에 대한 기업 개요 정보를 저장할 딕셔너리를 초기화합니다.

        complex_summary_table = soup.find('table', {'summary': '기업개요'})
        ...
        for row in complex_summary_data:
            ...
            company_summery[data_list[0]] = data_list[1]
            try:
                company_summery[data_list[2]] = data_list[3]
            except IndexError:
                continue
